...
--Starting--
local UI = Module.new("Bluesense | Universal",{
    Style = "Out",
    Themes = getgenv().Themes,
    Information = "Welcome to Bluesense | "..plr.Name
})
...
Home:AddButton("Developer V3rmillion | @Play Station 3",function() Copy("Play Station 3") end)
...
--Game-Info--
local Tab_Info = UI.new("Game-Info")

Tab_Info:AddButton("Rejoin Server",function()
...
Tab_Info:AddButton("Server Hop",function()
    UI:Notify(nil,"Finding server..") wait(.5) 
    Utility:ServerHop() 
    wait(1.5)
    UI:Notify(nil,"Failed To Serverhop | Reasons : No servers were available, Servers ping were too high or Servers were full")
end)
...
coroutine.wrap(function()
    while wait(2.5) do
        local Messages = {"Welcome to Bluesense | "..plr.Name,"Note : Your Settings Save (ESP Enabled,Aimbot,Boxes .Etc)","Find the source here | github.com/derek38/Releases","My Favorite Food Combination is Rice,Chicken and Egg"}
